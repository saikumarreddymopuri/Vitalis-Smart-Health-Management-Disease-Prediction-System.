...
const UserDashboard = () => {
  ...
  return (
    <div className="min-h-screen pt-20 pb-16 bg-white text-black dark:bg-gray-900 dark:text-white transition-colors duration-300">
      <Header
      ...
      <main className={`p-6 flex-grow transition-all duration-300 ${isOpen ? "ml-64" : "ml-0"}`}>
        {activeTab === "" && (
          <div className="flex flex-col items-center justify-center h-[70vh] text-center">
            ...
            <p className="text-gray-700 dark:text-gray-300">We’re glad to have you on board!</p>
          </div>
        )}

        {activeTab === "profile" && (
  <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-md max-w-md mx-auto transition-all duration-300">
    ...
    <div className="space-y-4 text-sm sm:text-base">
      ...
      <div className="flex justify-between border-b pb-2 border-gray-200 dark:border-gray-600">
        ...
        <span className="capitalize px-2 py-1 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 rounded">
          {user.role}
        </span>
      </div>
    </div>
            ...
            {!selectedAmbulanceInfo && (
              <div className="mt-6">
                ...
                {userAmbulanceBookings.length === 0 ? (
                  <p className="text-sm text-gray-700 dark:text-gray-300">No bookings yet.</p>
                ) : (
                  <ul className="space-y-2">
                    {userAmbulanceBookings.map((booking) => (
                      <li
                        key={booking._id}
                        className="p-3 border rounded bg-gray-100 dark:bg-gray-800 text-sm flex justify-between items-center"
                      >
                        <div>
                          <p>
                            <strong>Hospital:</strong> {booking.hospital.name}
                          ...
                          <p>
                            <strong>Ambulance Vehicle No: </strong>{" "}
        ...
        {activeTab === "history" && (
          <div className="bg-gray-200 p-6 dark:bg-gray-700 rounded shadow">
            <h2 className="text-xl font-bold dark:text-pink-500 mb-4">Your History</h2>
            ...
            {history.length === 0 ? (
              ...
              <div className="space-y-4">
                {history.map((item) => (
                  <div
                    key={item._id}
                    className="border border-gray-300 rounded p-4 bg-gray-50 dark:bg-gray-800 shadow-sm"
                  >
